...
{  
    ...
    app::Player* ResponseApi::Authorization(const StringRequest& req, StringResponse&& res)
    {
        std::string token = "";
    ...
    ResponseApi::StringResponse ResponseApi::Tick(const StringRequest& req)
    {
        ...
        try
        {
            ...
            time = json::value_to<int>(value.at("timeDelta"));
        }
        catch(...)
        {
        ...
        auto game_sessions_ = game_.GetGameSessions();

        for (auto game_session : game_sessions_)
        {
            auto dogs_ = game_session.GetDogs();

            for (auto dog_ : dogs_)
            {
                dog_.SetTime(std::chrono::milliseconds(time));
            }
        }
    ...
    ResponseApi::StringResponse ResponseApi::PlayerAction(const StringRequest& req)
    {
        ...
        auto dog_ = game_.FindGameSession(game_session_id_)->FindDog(player_->GetDogId());
    ...
    ResponseApi::StringResponse ResponseApi::State(const StringRequest& req)
    {
        ...
        for (const auto& player : allPlayers)
        {
            ...
            players_array_.emplace(std::to_string(player.second->GetId()), obj);
        }
    ...
    ResponseApi::StringResponse ResponseApi::Player(const StringRequest& req)
    {
        ...
        for (size_t i = 0; i < all_dogs_.size(); i++)
        {   
            json::object name;
            name["name"] = *all_dogs_.at(i).GetId();     
            
            obj.emplace(std::to_string(i), name);
        }
