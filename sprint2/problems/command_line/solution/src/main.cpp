...
namespace 
{    
    model::Game CreateNewGame(const bool is_debug, const bool default_spawn_, const std::filesystem::path& json_path)
    {
        model::Game game{is_debug, default_spawn_};

        json_loader::LoadGameData(std::move(game), json_path);
...
int main(int argc, const char* argv[]) {
    ...
    try {
        ...
        model::Game game = CreateNewGame(args->tick_period == 0, args->randomize_spawn_points, args->config_file);
        ...
        auto handler = std::make_shared<http_handler::RequestHandler>(game, players, args->www_root, api_strand);
