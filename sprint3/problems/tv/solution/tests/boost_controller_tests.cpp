 ...
 */
BOOST_AUTO_TEST_CASE(on_Info_prints_new_current_channel) {
    tv.SelectChannel(13);
    RunMenuCommand("Info"s);
    ExpectOutput("TV is turned on\nChannel number is 13\n"sv);
}

BOOST_AUTO_TEST_CASE(on_Info_prints_previus_channel) {
    RunMenuCommand("SelectChannel 42"s);
    RunMenuCommand("SelectChannel 13"s);
    RunMenuCommand("SelectPreviousChannel"s);
    ExpectOutput("42"sv);
}

BOOST_AUTO_TEST_CASE(on_Info_prints_min_error_channel) {
    RunMenuCommand("SelectChannel -1"s);
    ExpectOutput("Channel out of range!\n"sv);
}

BOOST_AUTO_TEST_CASE(on_Info_prints_max_error_channel) {
