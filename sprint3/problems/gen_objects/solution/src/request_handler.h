...
namespace http_handler
{
    ...
    public:
        ...
        RequestHandler(model::Game &game, app::Players &players, add_data::GameLoots &game_loots, std::string game_file_path, Strand api_strand)
            : game_{game}, players_{players}, game_loots_{game_loots}, game_file_path_(game_file_path), api_strand_{api_strand}
        {
        }
        ...
        void operator()(http::request<Body, http::basic_fields<Allocator>> &&req, Send &&send)
        {
            ...
            {
                auto handle = [self = shared_from_this(), send, req = std::forward<decltype(req)>(req), str]
                {
                    auto api = std::make_shared<ResponseApi>(self->game_, self->players_, self->game_loots_);
